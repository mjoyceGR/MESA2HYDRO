...
def make_IC_binary(fname, mp, central_point_mass,\
    ...
    total_number_of_particles=len(x)#+1  ## mjoyce 11/2/2018  for central mass
    ...
    my_body.hsml[:]=0.*total_number_of_particles + (-1)  #shazrene says -1 but that's not working at all
...
def make_IC_text(\
    ...
    print >> outf, ("%.32f"%central_point_mass)\
          +' '+("%.32f"%0)\
          +' '+("%.32f"%0)\
          +' '+("%.32f"%0)\
          +' '+("%.0f"%0)
...
#     p0 = file.create_group("PartType0")
